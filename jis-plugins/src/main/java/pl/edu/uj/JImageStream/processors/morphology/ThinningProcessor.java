...
public class ThinningProcessor implements ImageProcessor {
    ...
    public StreamableImage process(StreamableImage input) {
        ...
        input = input.stream().apply(new SubstractFilter(side180)).collect(StreamableImage::new);
        ...
        input = input.stream().apply(new SubstractFilter(edge180)).collect(StreamableImage::new);
        ...
        input = input.stream().apply(new SubstractFilter(side270)).collect(StreamableImage::new);
        ...
        input = input.stream().apply(new SubstractFilter(edge270)).collect(StreamableImage::new);
...
//                .collect(StreamableImage::new);
    ...
    private class SubstractFilter extends ArithmeticFilter {
        ...
        @Override
        protected Pixel calculate(Pixel x, Pixel y) {
            return new Pixel(x.getRed() - y.getRed(), x.getGreen() - y.getGreen(), x.getBlue() - y.getBlue(), 255);
        }
    }
